...
$globalid = mysqli_real_escape_string($conn,$_SESSION["globalidfordraft"]);
...
if (isset($_POST["push"])) {
    ...
    $sql = "UPDATE `service_requests` SET `cycle`='1',`role`='$role',`skilllevel`='$skilllevel',`location`='$location',`skillset`='$skillset',`duration`='$duration',`projectname`='$projectname',`taskdescription`='$taskdescription',`comments`='$comments',`weight`='$weight',`created_at`=current_timestamp,`created_by`='$username',`updated_at`=current_timestamp,`Submission_status`='1',`deadline`='$deadline',`expired_status`='0' WHERE globalid = '$globalid'";
  	$result = mysqli_query($conn, $sql);
    $check = mysqli_query($conn, "SELECT * FROM service_requests WHERE globalid ='$globalid' AND Submission_status = '1' ");

  if (mysqli_num_rows($check)>0) {
          echo "<script>alert('Draft submitted successfully');</script>";
    } else {
...
<body>
   ...
   <?php
	$sql = "SELECT * FROM service_requests WHERE globalid = '$globalid' AND Submission_status='0' "; 
    ...
    <div class="container-fluid">
        <div class="request-form-wrapper form-hide js-request-form-wrapper">
            <div class="form-head">
                <h1>Create Service Request Form from Saved draft</h1>
            </div>
                ...
                <div class="form-inputs">
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            ...
                            value="<?php print_r($field0); ?>" value="<?php echo $_POST["deadline"]; ?>" required />
                        </div>
                    </div>
                    <div class="mb-3 row">
                    ...
                    <div class="form-input-actions">
                        <div id="actionButtons">
                            <input type="submit" class="btn" name="push" value="Submit Draft" />
                        </div>
                    </div>
                </div>
            </form>
